...
Scopus document results: 2208 results. Exported from Scopus through 2 .csv that are going to be merged here. Then merged with included reviews and deduplicated. The included reviews are 175 and not 109 because the file includes some of the non-systematic reviews that I removed for mappign and appraisal.
...
Loading new files
```{r, include=FALSE}
dat1 <- read.csv(here("data/update/scopus1.csv"))
dim(dat1) #[1] 111   7
```
```{r}
dat2 <- read.csv(here("data/update/scopus2.csv"))
dim(dat2) #[1] 1305   7
```
...
```{r}
dat3 <- read.csv(here("data/update/articles174.csv"))
dim(dat3) #[1] 174   7
```
