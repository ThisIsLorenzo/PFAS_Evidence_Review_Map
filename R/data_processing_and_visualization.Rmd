...
mdata <- 
  ...
  mutate(Journal = ifelse(Journal == "International journal of environmental research and public health", "International Journal of Environmental Research and Public Health", Journal)) %>% 
  mutate(Journal = ifelse(Journal == "Environmental research", "Environmental Research", Journal)) # Fix one journal title
...
#Merge main data with PFAS data table
...
#create two new columns in mdata table called 'First_author' and 'Publication_year', containing the first and second elements of the 'Author_year' column, respectively.
mdata2 <- mdata %>% 
...
Make the same table for all reviews included in bibliometric analysis
```{r}
mdata_biblio <- 
...
Cumulative time-trends grouped by review type:

```{r}
mdata2 <- 
  mdata %>%
  separate_rows(Review_type_claimed, sep=";")

mdata2$review_type <- ifelse(mdata2$Review_type_claimed == "systematic review"|
                              mdata2$Review_type_claimed == "meta-analysis" ,
                              mdata2$Review_type_claimed, "systematic review")
...
trend_by_review <- review_trend %>%
  ggplot(aes(x = Publication_year,
             y = cumulative,
             color = fct_relevel(review_type, c("systematic review",
                                                          "meta-analysis")))) +
  geom_line(size = 0.7) + 
  geom_point(aes(color = review_type), size = 3, shape = 21, fill = "white") +
  geom_text(aes(label = n, x = Publication_year, y = cumulative), 
            size = 3.5, 
            hjust = 1.2,
            vjust = -0.5) +
...
Cumulative time-trends grouped by broad taxa:
...
Cumulative time-trends grouped by top 10 journals:

```{r}
mdata$Journal <- tolower(mdata$Journal)
...
time_trend_journals <- top_journals %>%
  ggplot(aes(x = Publication_year,
             y = cumulative,
             color = fct_relevel(Journal, c("Mammals",
                                                          "Birds",
                                                          "Ray-finned fish",
                                                          "Others")))) +
  geom_line(size = 0.7) + 
...
Cumulative time-trends grouped by top 10 journals and others
...
p_PFAS_focus <- 
  ...
  theme_light() +
  coord_flip() +
  # geom_text(aes(label = paste0(round((n/sum(n))*100,1),'%'), x = PFAS_focus), 
  #           position = position_stack(vjust = 0.5), 
  #           size = 3.5) +
  ggtitle(expression(bold("Is the review focused on PFAS?"))) +
...
Time-trend of number of publications focused on PFAS vs not focused on PFAS
...
t_pfas_focus <-  pfas_focus %>%
  ggplot(aes(x = Publication_year,
             y = cumulative,
             color = fct_relevel(PFAS_focus, c("Yes",
                                                          "No")))) +
  geom_line(size = 0.7) + 
  geom_point(aes(color = PFAS_focus), size = 3, shape = 21, fill = "white") +
  geom_text(aes(label = n, x = Publication_year, y = cumulative), 
            size = 3.5, 
            hjust = 1.2,
            vjust = -0.5) +
...
Time-trend of number of publications investigating each PFAS type
...
t_pfas_types <-  trend_pfas_types %>%
  ggplot(aes(x = Publication_year,
             y = cumulative,
             color = reorder(PFAS_type, -cumulative))) +
  geom_line(size = 0.7) + 
  geom_point(aes(color = reorder(PFAS_type, -cumulative)), size = 3, shape = 21, fill = "white") +
  geom_text(aes(label = n, x = Publication_year, y = cumulative), 
            size = 3.5, 
            hjust = 1.2,
            vjust = -0.5) +
...
Time-trend of number of publications by PFAS group
...
t_pfas_group <-  trend_pfas_groups %>%
  ...
  geom_line(size = 0.7) + 
  geom_point(aes(color = reorder(PFAS_group, -cumulative)), size = 3, shape = 21, fill = "white") +
  geom_text(aes(label = n, x = Publication_year, y = cumulative), 
            size = 3.5, 
            hjust = 1.2,
            vjust = -0.5) +
...
Time-trend of number of publications by PFAS group - semplified
...
t_pfas_group2 <-  trend_pfas_groups2 %>%
  ggplot(aes(x = Publication_year,
             y = cumulative,
             color = reorder(PFAS_group2, -cumulative))) +
  geom_line(size = 0.7) + 
  geom_point(aes(color = reorder(PFAS_group2, -cumulative)), size = 3, shape = 21, fill = "white") +
  geom_text(aes(label = n, x = Publication_year, y = cumulative), 
            size = 3.5, 
            hjust = 1.2,
            vjust = -0.5) +
...
p_mesh_headings <- ggplot(MeSH_headings_counts[1:10, ], aes(x = MeSH_headings, y = n)) +
  geom_col(aes(fill = ""), width = 0.7) +
  theme_light() +
...
p_subj <- ggplot(t_subject, aes(x = Human_animal_environment, y = n)) +
  geom_col(aes(fill = ""), width = 0.7) +
  theme_light() +
...
Time-trend of SRs publications on main review subjects
...
t_pfas_subjects <-  trend_subjects %>%
  ggplot(aes(x = Publication_year,
             y = cumulative,
             color = reorder(Human_animal_environment, -cumulative))) +
  geom_line(size = 0.7) + 
  geom_point(aes(color = reorder(Human_animal_environment, -cumulative)), size = 3, shape = 21, fill = "white") +
  geom_text(aes(label = n, x = Publication_year, y = cumulative), 
            size = 3.5, 
            hjust = 1.2,
            vjust = -0.5) +
...
p_mesh_subject <- ggplot(t_topic_subject, aes(x = MeSH_qualifiers, 
                            y = n)) +
 theme_light() +
 labs(title = expression("MeSH qualifiers and subjects reviewed")) +
 coord_flip()  +
 scale_fill_brewer() +
...
p1 <-
  ...
  geom_text(aes(label = paste0(round((n/sum(n))*100,1),'%'), x = COI_statement), 
            position = position_stack(vjust = 0.5), 
            size = 3.5) + 
  ggtitle(expression(bold("a)") ~ " Is the COI statement provided?"))
...
p2 <- 
  ...
  geom_text(aes(label = paste0(round((n/sum(n))*100,1),'%'), x = COI_present), 
            position = position_stack(vjust = 0.5),
            size = 3.5) +
  ggtitle(expression(bold("b)") ~ " Is potential COI acknowledge?"))
p2
```
...
p3 <- 
  ...
  geom_text(aes(label = paste0(round((n/sum(n))*100,1),'%'), x = Funding_statement), 
            position = position_stack(vjust = 0.5), 
            size = 3.5) +
  ggtitle(expression(bold("a)") ~ " Is the funding statement provided?"))
...
p4 <- 
  ...
  geom_text(aes(label = paste0(round((n/sum(n))*100,1),'%'), x = Non_profit_funding), 
            position = position_stack(vjust = 0.5),
            size = 3.5) +
  ggtitle(expression(bold("b)") ~ " Fundings from non-profit sector"))
...
p5 <- 
  ...
  geom_text(aes(label = paste0(round((n/sum(n))*100,1),'%'), x = Industry_funding),
            position = position_stack(vjust = 0.5),
            size = 3.5) +
  ggtitle(expression(bold("c)") ~ " Fundings from industry sector"))
...
p6 <- ggplot(t_Raw_data, aes(x = Raw_data, 
  ...
  geom_text(aes(label = paste0(round((n/sum(n))*100,1),'%'), x = Raw_data),
            position = position_stack(vjust = 0.5),
            size = 3.5) +
  ggtitle(expression(bold("a)") ~ " Are review's data available?"))
...
p7 <- ggplot(t_Analysis_code, aes(x = Analysis_code, 
  ...
  geom_text(aes(label = paste0(round((n/sum(n))*100,1),'%'), x = Analysis_code),
            position = position_stack(vjust = 0.5), 
            size = 3.5) +
  ggtitle(expression(bold("b)") ~ " Is the review's analysis code available?"))
p7
```
