...
cbp2 <- c("#A29F15",#Green is often associated with nature and life, making it fitting for the category of humans
          ...
          "#CCCCCC")#Grey is a neutral color
...
ggsave(here("R_data", "plot_subjects_mesh_combined.png"),
       width = 8,
       height = 6)
```
...
ggsave(here("R_data", "plot_pfas_combined.png"),
       width = 8,
       height = 6)
```
...
p1 <-
  ...
  geom_col(aes(fill = fct_relevel(COI_statement,
                                  c("Yes", "No")),
                                  width = 0.7)) +
  scale_fill_manual(values = c("#7A7A7A", "#CCCCCC"),
                    breaks = c("Yes", "No")) +
  scale_y_continuous(name = "Article count") +
...
p3 <- 
  ...
  geom_text(aes(label = paste0(round((n/sum(n))*100,1)), x = Funding_statement), 
            position = position_stack(vjust = 0.5), 
            size = 3.5) +
  ggtitle(expression(bold("c)") ~ " Is the funding statement provided?"))
...
p4 <- 
  ...
  geom_text(aes(label = paste0(round((n/sum(n))*100,1)), x = Non_profit_funding), 
            position = position_stack(vjust = 0.5),
            size = 3.5) +
  ggtitle(expression(bold("d)") ~ " Fundings from non-profit sector"))
...
p5 <- 
  ...
  geom_text(aes(label = paste0(round((n/sum(n))*100,1)), x = Industry_funding),
            position = position_stack(vjust = 0.5),
            size = 3.5) +
  ggtitle(expression(bold("e)") ~ " Fundings from industry sector"))
...
p6 <- ggplot(t_Raw_data, aes(x = Raw_data, 
  ...
  geom_text(aes(label = paste0(round((n/sum(n))*100,1)), x = Raw_data),
            position = position_stack(vjust = 0.5),
            size = 3.5) +
  ggtitle(expression(bold("f)") ~ " Are review's data available?"))
...
p7 <- ggplot(t_Analysis_code, aes(x = Analysis_code, 
  ...
  geom_text(aes(label = paste0(round((n/sum(n))*100,1)), x = Analysis_code),
            position = position_stack(vjust = 0.5), 
            size = 3.5) +
  ggtitle(expression(bold("g)") ~ " Is the review's analysis code available?"))
p7
```
...
```{r}
pp1+pp2+pp3
# ggsave(here("R_data", "plots_transparency_combined.png"),
#        width = 8,
#        height = 6)
```
