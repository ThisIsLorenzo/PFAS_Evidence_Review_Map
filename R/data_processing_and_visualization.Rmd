...
publ_year <- mdata %>% 
  ...
  scale_y_continuous(name = "Number of published systematic reviews",
                    ...
                    minor_breaks = NULL) +
  ggtitle("Time trend") + #adds a title to the plot
...
```{r}
#TODO - Remove plants. Exclude the review? (S_070, Wang_2020)
...
t_pfas_focus2 <-  pfas_focus2 %>%
  ggplot(aes(x = Publication_year,
             y = cumulative,
             color = reorder(PFAS_one_many, -cumulative))) +
  geom_line(size = 0.7) + 
  geom_point(aes(color = PFAS_one_many), size = 3, shape = 21, fill = "white") +
  geom_text(aes(label = n, x = Publication_year, y = cumulative), 
            size = 3.5, 
            hjust = 1.2,
            vjust = -0.5) +
...
t_pfas_ccc <-  trend_pfas_ccc %>%
  ggplot(aes(x = Publication_year,
             y = cumulative,
             color = reorder(PFAS_carbon_chain, -cumulative))) +
  geom_line(size = 0.7) + 
  geom_point(aes(color = reorder(PFAS_carbon_chain, -cumulative)), size = 3, shape = 21, fill = "white") +
  geom_text(aes(label = n, x = Publication_year, y = cumulative), 
            size = 3.5, 
            hjust = 1.2,
            vjust = -0.5) +
...
t_guideline <-  rep_guideline %>%
  ...
  geom_line(size = 0.7) + 
  geom_point(aes(color = Reporting_guideline), size = 3, shape = 21, fill = "white") +
  geom_text(aes(label = n, x = Publication_year, y = cumulative), 
            size = 3.5, 
            hjust = 1.2,
            vjust = -0.5) +
...
t_coi <-  coi_statement %>%
  ...
  geom_line(size = 0.7) + 
  geom_point(aes(color = COI_statement), size = 3, shape = 21, fill = "white") +
  geom_text(aes(label = n, x = Publication_year, y = cumulative), 
            size = 3.5, 
            hjust = 1.2,
            vjust = -0.5) +
...
t_funding <-  funding_statement %>%
  ...
  geom_line(size = 0.7) + 
  geom_point(aes(color = Funding_statement), size = 3, shape = 21, fill = "white") +
  geom_text(aes(label = n, x = Publication_year, y = cumulative), 
            size = 3.5, 
            hjust = 1.2,
            vjust = -0.5) +
...
funding1 <- mdata %>% 
  ...
  group_by(Non_profit_funding, Industry_funding) %>%
  count(Publication_year) %>% 
  mutate(cumulative = cumsum(n))
...
t_PFAStype$percent <- round(t_PFAStype$percent, digits = 0)
...
PFAS_types <- ggplot(t_PFAStype, aes(x = PFAS_type,
  ...
  scale_fill_manual(values = c("#919191")) +
  geom_text(aes(label = percent), hjust = -0.2, size = 2.7) +
  theme(legend.position = "none",
...
#count percent frequencies of headings
MeSH_headings_counts <- t_MESH2 %>%
  filter(MeSH_headings != "NA") %>%
  count(MeSH_headings) %>% 
  mutate(percent = (n/109 * 100))
  
MeSH_headings_counts$percent <- round(MeSH_headings_counts$percent, digits = 0)

MeSH_headings_counts$MeSH_headings <- factor(MeSH_headings_counts$MeSH_headings, levels = MeSH_headings_counts$MeSH_headings[order(MeSH_headings_counts$percent, decreasing = FALSE)])

MeSH_headings_counts <- MeSH_headings_counts %>% 
  top_n(10, percent)

MESH_headings_order <- 
  MeSH_headings_counts$MeSH_headings[order(MeSH_headings_counts$percent, decreasing = FALSE)] #save for the next plot

# simple barplot with top 10 MESH headings
p_mesh_headings <- ggplot(MeSH_headings_counts, aes(x = MeSH_headings, y = percent)) +
  geom_col(aes(fill = ""), width = 0.7) +
  theme_light() +
  labs(title = expression("Top 10 MeSH headings reviewed")) +
  coord_flip() +
  scale_y_continuous(name = "Percent (%)",
...
t_subject$percent <- round(t_subject$percent, digits = 0)
...
p_subj <- ggplot(t_subject, aes(x = Human_animal_environment, y = percent)) +
  geom_col(aes(fill = ""), width = 0.7) +
  theme_light() +
