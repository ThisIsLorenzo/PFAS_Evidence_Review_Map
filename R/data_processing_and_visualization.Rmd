...
ggplot(t_PFAStype, aes(x = PFAS_type, y = n)) +
  ...
  theme(legend.position = "none", axis.title.x = element_text(size = 11), axis.title.y = element_blank(), axis.text.y = element_text(size = 7))
...
t_MESH2 %>% select(MeSH_headings) %>% count(MeSH_headings) %>% arrange(desc(n)) %>% 
  filter(MeSH_headings != "NA") %>% filter(MeSH_headings != "") ->  MeSH_headings_counts 
...
ggplot(MeSH_headings_counts[1:10, ], aes(x = MeSH_headings, y = n)) +
  ...
  scale_y_continuous(name = "Article count") +
  scale_fill_manual(values = c("#919191")) +
  theme(legend.position = "none", axis.title.x = element_text(size = 11), axis.title.y = element_blank())
...
ggplot(MeSH_qualifiers_counts[, ], aes(x = MeSH_qualifiers, y = n)) +
  ...
  scale_y_continuous(name = "Article count") +
  scale_fill_manual(values = c("#919191")) +
  theme(legend.position = "none", axis.title.x = element_text(size = 10), axis.title.y = element_blank(), axis.text = element_text(size = 7))
...
ggplot(t_topic_subject, aes(x = MeSH_qualifiers, y = n)) +
 ...
 geom_col(aes(fill = fct_relevel(Subject, c("Mixed", "Plants", "Environment", "Animals", "Humans" ))), width = 0.7) + 
   scale_fill_manual(values=cbp1, breaks=c("Humans", "Animals", "Environment", "Plants", "Mixed"))+
 scale_y_continuous(name = "Article count") +
 theme_light() +
 theme(legend.position = "bottom", legend.box.background = element_rect(colour = "black"), legend.title = element_blank(),  legend.text = element_text(size = 20), axis.title.x = element_text(size = 18), axis.title.y = element_blank(), axis.text = element_text(size = 18)) 
...
mdata %>% count(Publication_year) %>% ggplot(aes(x = Publication_year, y = n)) + 
  ...
  theme_minimal() +
  scale_x_continuous(name = "", breaks = seq(2008, 2022, by = 1)) +
  scale_y_continuous(name = "Article count", breaks = seq(0, 50, by = 2), sec.axis = sec_axis(~ . *1, name = "Article count", breaks = seq(0, 50, by = 2))) +
  ggtitle("Publication year") + 
  theme(plot.title = element_text(hjust = 0.5), axis.text.x = element_text(angle = 45, size = 9, margin = margin(t = 5, r = 0, l = 0)), axis.text.y = element_text(size = 10, margin = margin(t = 0, r = 5, l = 5)))
...
qtable_long <- data.frame(study = studies, question = rep(colnames(qtable), each = length(studies)), score = as.vector(qtable), stringsAsFactors = TRUE) #make long format table
...
p1 <- ggplot(t_COI_statement, aes(x = COI_statement, y = n)) +
 theme_light() +
...
p2 <- ggplot(t_COI_present, aes(x = COI_present, y = n)) +
 theme_light() +
 theme(legend.position = "none", axis.title.y = element_blank()) +
...
p3 <- ggplot(t_Funding_statement, aes(x = Funding_statement, y = n)) +
 theme_light() +
 theme(legend.position = "none", axis.title.y = element_blank()) +
...
p4 <- ggplot(t_Nonprofit_funding, aes(x = Nonprofit_funding, y = n)) +
 theme_light() +
 theme(legend.position = "none", axis.title.y = element_blank()) +
...
p5 <- ggplot(t_Funding_industry, aes(x = Industry_funding, y = n)) +
 theme_light() +
 theme(legend.position = "none", axis.title.y = element_blank()) +
...
NetMatrix_keywords_plot <- networkPlot(NetMatrix_keywords, normalize="association", n = 30, 
                                       Title = "Keyword co-occurrences", type = "fruchterman", 
                                       size.cex = TRUE, size = 15, remove.multiple = F, 
                                       edgesize = 4, labelsize = 3, label.cex = TRUE, 
                                       edges.min = 2, label.n = 30, alpha = 0.3) #adjust number and size of labels, as needed, etc.
#dev.off()
