...
p_PFAS_focus <- 
  ...
  theme_light() +
  coord_flip() +
  geom_text(aes(label = ifelse(n>0,as.character(n),''), x = PFAS_focus), 
...
# t_PFASfocus_label <- t_PFASfocus %>%
#   group_by(PFAS_focus, PFAS_one_many) %>%
#   summarize(n = sum(n)) %>%
#   mutate(label = paste(PFAS_one_many, n, sep = ": ")) %>% 
#   arrange(PFAS_focus, PFAS_one_many)
# 
# p_PFAS_focus + geom_text(data = t_PFASfocus_label, aes(label = label, fill = PFAS_one_many), position = position_stack(vjust = 0.5))
...
publ_year <- mdata %>%
  ...
  ggplot(aes(x = Publication_year,
             y = cumulative)) + 
  geom_area(fill = '#ADADAD', 
            alpha = 0.5) +
  geom_line(color = '#5C5C5C', 
            size = 1) + 
...
mdata_classes <- left_join(mdata, spdata, by = "Study_ID") %>%
  filter(!is.na(Class_scientific_name)) %>% 
  group_by(Class_scientific_name)
...
p1 <-
  ...
  theme_light() +
  coord_flip() +
  geom_text(aes(label = n, x = COI_statement), 
            position = position_stack(vjust = 0.5), 
            size = 3.5) + 
...
p2 <- 
  ...
  coord_flip() +
  theme_light() +
  geom_text(aes(label = n, x = COI_present), 
            position = position_stack(vjust = 0.5),
            size = 3.5) +
...
p3 <- 
  ...
  theme_light() +
  coord_flip() +
  geom_text(aes(label = n, x = Funding_statement), 
            position = position_stack(vjust = 0.5), 
            size = 3.5) +
...
p4 <- 
  ...
  theme_light() +
  coord_flip() +
  geom_text(aes(label = n, x = Non_profit_funding), 
            position = position_stack(vjust = 0.5),
            size = 3.5) +
...
p5 <- 
  ...
  theme_light() +
  coord_flip() +
  geom_text(aes(label = n, x = Industry_funding),
            position = position_stack(vjust = 0.5),
            size = 3.5) +
...
p6 <- ggplot(t_Raw_data, aes(x = Raw_data, 
  ...
  theme_light() +
  coord_flip() +
  geom_text(aes(label = n, x = Raw_data),
            position = position_stack(vjust = 0.5),
            size = 3.5) +
...
p7 <- ggplot(t_Analysis_code, aes(x = Analysis_code, 
  ...
  theme_light() +
  coord_flip() +
  geom_text(aes(label = n, x = Analysis_code),
            position = position_stack(vjust = 0.5), 
            size = 3.5) +
