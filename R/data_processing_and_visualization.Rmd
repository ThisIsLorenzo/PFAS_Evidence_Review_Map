...
p4 <- 
  ggplot(t_Nonprofit_funding, aes(x = Nonprofit_funding,
                                  y = n)) +
  theme(legend.position = "none",
        axis.title.y = element_blank()) +
  geom_col(aes(fill = fct_relevel(Nonprofit_funding,
                                  c("Yes", "No")), width = 0.7)) + 
  scale_fill_manual(values = cbp1,
                    breaks = c("Yes", "No")) + 
  scale_y_continuous(name = "Article count") +
  ...
  theme_light() +
  coord_flip() +
  geom_text(aes(label = n, x = Nonprofit_funding), 
            y = max(t_Nonprofit_funding$n)/2,
            hjust = 0,
            size = 3.5) +
