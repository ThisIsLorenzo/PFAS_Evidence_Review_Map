...
p_PFAS_focus <- 
  ...
  geom_text(aes(fill = fct_relevel(PFAS_one_many, c("Not specified",
                                                    "Multiple", 
                                                    "One")),
                label = paste0(round((n/sum(n))*100,1),'%'), x = PFAS_focus),
...
PFAS_types <- ggplot(t_PFAStype, aes(x = PFAS_type,
  ...
  scale_fill_manual(values = c("#919191")) +
  geom_text(aes(label = paste0(round((n/109)*100,1),'%'), x = PFAS_type), 
            position = position_stack(vjust = 0.5), 
            size = 3) + 
  theme(legend.position = "none",
...
# change to long format (one species per row - one or multiple rows per study):
t_MESH <-
  mdata %>%
  select(MeSH_terms) %>%
...
p_mesh_headings <- ggplot(MeSH_headings_counts, aes(x = MeSH_headings, y = n)) +
  geom_col(aes(fill = ""), width = 0.7) +
  theme_light() +
  labs(title = expression("Top 10 MeSH headings")) +
  coord_flip() +
  scale_y_continuous(name = "Headings count",
                     breaks = seq(0, 90, by = 20)) +
  scale_fill_manual(values = c("#919191")) +
  geom_text(aes(label = paste0(round((n/sum(n))*100,1),'%'), x = MeSH_headings), 
            position = position_stack(vjust = 0.5), 
            size = 3.5) +
  theme(legend.position = "none",
        axis.title.x = element_text(size = 11),
...
p_mesh_qualifiers <- ggplot(MeSH_qualifiers_counts, aes(x = MeSH_qualifiers,
  ...
  geom_col(aes(fill = ""), 
           ...
           position = 'dodge') +
  theme_light() +
  labs(title = expression("MeSH qualifiers reviewed")) +
  coord_flip() +
  scale_y_continuous(name = "Qualifiers count",
                     breaks = seq(0, 110, by = 20)) +
  scale_fill_manual(values = c("#919191")) +
  geom_text(aes(label = paste0(round((n/sum(n))*100,1),'%'), x = MeSH_qualifiers), 
            position = position_stack(vjust = 0.5), 
            size = 3.5) +
  theme(legend.position = "none",
...
p_subj <- ggplot(t_subject, aes(x = Human_animal_environment, y = n)) +
  ...
  scale_fill_manual(values = c("#919191")) +
  geom_text(aes(label = paste0(round((n/sum(n))*100,1),'%'), x = Human_animal_environment), 
            position = position_stack(vjust = 0.5), 
            size = 3.5) + 
  theme(legend.position = "none",
        axis.title.x = element_text(size = 10),
...
## MeSH qualifiers & main review subjects
...
p_PFAStype <-
  ...
  geom_text(aes(fill = fct_relevel(Human_animal_environment, c("Mixed",
                                                    ...
                                                    "Animals",
                                                    "Humans")),
                label = paste0(round((n/109)*100,1)), x = PFAS_type),
            position = position_stack(vjust = 0.5),
            size = 1.8) +
