...
t_spfocus <-
  ...
  filter(Species_one_many != "NA",
         Class_scientific_name != "NA")  #filter out NA

ggplot(t_spfocus, aes(x = Class_scientific_name,
                      y = n)) +
 theme_light() +
 labs(title = expression("Taxonomic classes focus")) + #~bold(A.)~' Type and subject'
 coord_flip()  +
 scale_fill_brewer() +
...
t2 <- mdata %>%
  count(Species_one_many, 
        PFAS_one_many) #make table of counts across two columns 

t2$Species_one_many <- as.factor(t2$Species_one_many)

levels(t2$Species_one_many) <- c("Many", 
                                 "One") #reorder levels for Species_one_many

t2 <- t2 %>%
  filter(Species_one_many != "NA") #filter out NA

# simple dot plot
ggplot(data = t2, aes(Species_one_many, 
                      PFAS_one_many)) + geom_point(aes(size = n), colour = "darkgrey") +
theme_classic() +
...
NetMatrix_authors <- 
  biblioNetwork(bib_sco3, analysis = "collaboration",  network = "authors", sep = ";")
