...
mpti <- left_join(mdata, ptidata, by = "Study_ID")
```

# Create a summary of included studies
```{r summary included studies}
knitr::kable(select(mdata, Study_ID,	Author_year,	Paper_title), caption = "Table of included studies")
```

# OBJECTIVE 1: Mapping evidence on PFAS

#Do reviews investigate one or multiple PFAS types? 
```{r}
mdata %>% count(PFAS_focus, PFAS_one_many) %>% arrange(desc(n)) %>% 
  filter(PFAS_focus != "NA") -> t_PFASfocus
