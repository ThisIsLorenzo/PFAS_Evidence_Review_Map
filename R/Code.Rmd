...
First, we remove unnecessary columns for analysis. These columns will be reported in the final extraction sheet.

```{r removing unnecessary columns}
mdata <- mdata %>% select(- ends_with("comment")) %>% 
  select("Study_ID":"DOI")
qdata <- qdata %>% select(- ends_with("comment")) %>% 
  select("Study_ID":"Q16") # I WANT TO REDUCE IT AS ONLY "Q16"
...
### Clean AMSTAR2 checklist data - remove description of values
```{r clean AMSTAR2 checklist}
unique(qdata$Q1)
```
